...
ZeroCrossing<SampleType>::ZeroCrossing - Bridge-header between Juce and the TimbreID zeroCrossing method
...
template <typename SampleType>
ZeroCrossing<SampleType>::ZeroCrossing(){ resizeBuffers(); reset(); }

template <typename SampleType>
ZeroCrossing<SampleType>::ZeroCrossing(unsigned long int windowSize)
{
...
template <typename SampleType>
void ZeroCrossing<SampleType>::prepare (double sampleRate, uint32_t blockSize) noexcept
{
...
template <typename SampleType>
void ZeroCrossing<SampleType>::reset() noexcept
{
...
template <typename SampleType>
uint32_t ZeroCrossing<SampleType>::compute()
{
    #if ASYNC_FEATURE_EXTRACTION
...
template <typename SampleType>
void ZeroCrossing<SampleType>::setWindowSize(uint32_t windowSize)
{
...
template <typename SampleType>
uint32_t ZeroCrossing<SampleType>::getWindowSize() const
{
...
*/
template <typename SampleType>
std::string ZeroCrossing<SampleType>::getInfoString() const noexcept
{
    std::string res = "";
...
template <typename SampleType>
void ZeroCrossing<SampleType>::resizeBuffers()
{
...
template <typename SampleType>
void ZeroCrossing<SampleType>::storeAudioBlock (const SampleType* input, size_t n) noexcept
{
...
template class ZeroCrossing<float>;
template class ZeroCrossing<double>;
